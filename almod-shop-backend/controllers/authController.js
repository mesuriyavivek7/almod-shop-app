...
export const login=async (req,res,next)=>{
    try{
        ...
        res.cookie("access_token",token,{expires:new Date(Date.now()+2592000000),httpOnly:false,secure:true,sameSite:'None',domain:'.fuelflex.in'}).status(200).json({details:{...otherDetails},isAdmin})
    }catch(err){
        next(err)
    }
}
...
export const logout=async (req,res,next)=>{
    try{
        res.clearCookie('access_token', { httpOnly: true, secure: true, sameSite: 'None',domain:'fuelflex.in' })
